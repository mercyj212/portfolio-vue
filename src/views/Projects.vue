<template>
    <nav class="bg-gray-800 p-4">
      <div class="container mx-auto flex justify-between items-center">
        ...
        <ul class="flex space-x-4">
          ...
          <li><router-link to="/contact" class="text-gray-300 hover:text-white hover:bg-gray-900 px-3 py-3 rounded-md font-semibold"
              active-class="bg-gray-900 text-white"
              >Contact</router-link></li>

          <li><router-link to="/projects" class="text-gray-300 hover:text-white hover:bg-gray-900 px-3 py-3 rounded-md font-semibold"
              active-class="bg-gray-900 text-white"
              exact
    ...
    <div class="container mx-auto mt-10">
      ...
      <div v-if="loading" class="col-span-3 flex justify-center items-center h-32">
        <span class="animate-spin rounded-full h-8 w-8 border-t-2 border-b-2 border-gray-800 mr-3"></span>
        <span class="text-lg text-gray-600">Loading projects...</span>
      </div>
      <div
        v-for="project in projects"
        :key="project.id"
        class="bg-white p-8 rounded-xl shadow-lg hover:shadow-2xl transition duration-300 flex flex-col items-center"
      >
        <div class="w-16 h-16 mb-4 bg-gradient-to-tr from-blue-400 to-purple-500 rounded-full flex items-center justify-center text-white text-2xl font-bold shadow">
        {{ project.name.charAt(0) }}
        </div>
        <h2 class="text-2xl font-bold mb-2 text-gray-800">{{ project.name }}</h2>
        <p class="text-gray-600 mb-4">{{ project.description }}</p>
        <button class="mt-auto bg-blue-600 hover:bg-blue-700 text-white font-semibold py-2 px-6 rounded-lg transition">
        View Details
        </button>
      </div>
      </div>
